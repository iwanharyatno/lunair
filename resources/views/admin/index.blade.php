    ...
    <div class="modal fade" tabindex="-1" id="delete-dialog" data-bs-backdrop="static">
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal"
                        id="cancel-delete-button">Cancel</button>
                ...
                <form method="POST" action="/admin/add-product" id="product-data-form">
                    ...
                    <div class="modal-footer">
                        ...
                        <button type="submit" class="btn btn-primary" data-bs-toggle="modal"
                            data-bs-target="#add-image-modal">Next</button>
                    </div>
                </form>
            </div>
        </div>
                ...
                @foreach ($products as $product)
                    <div class="card col-md-3 mx-md-3 mb-3 py-2" id="product-{{ $product->id }}">
                        ...
                        <div class="card-body">
                            ...
                            <h6 class="card-subtitle text-muted mb-2">Stock: {{ $product->stock }} |
                                {{ numfmt_format_currency($fmt, $product->price, 'IDR') }}</h6>
                            <p class="card-text">{{ urldecode($product->description) }}</p>
                            <button class="btn btn-primary edit-button"
                                data-fields='{"name":"{{ $product->name }}","description":"{{ $product->description }}","price":{{ $product->price }},"stock":{{ $product->stock }},"id":"{{ $product->id }}","image_id":"{{ $product->image ? $product->image->id : -1 }}","category":"{{ $product->category }}"}'
                                data-mode="edit" data-bs-toggle="modal"
                                data-bs-target="#add-product-modal">Edit</button>
                            <button class="btn btn-danger" data-bs-toggle="modal" data-bs-target="#delete-dialog"
                                data-prod-id="{{ $product->id }}">Delete</button>
                        </div>
                    </div>
                @endforeach
                @if ($products->isEmpty())
                    ...
                    if (withFile) {
                        ...
                        xhr.addEventListener('load', function() {
                            if (xhr.status == 200) {
                                ...
                                if (!update) insertNewNode(data.product, JSON.parse(this.response)
                                    .image);
